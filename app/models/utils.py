...
class EarlyStopping:
    ...
    def __call__(self, i, loss, estimated_i, estimated_k=None, estimated_b=None):
        score = -loss

        if self.best_score is None:
            self.best_score = score
            self.save(i, loss, estimated_i, estimated_k, estimated_b)
        elif score < self.best_score:
            self.counter += 1
            if self.verbose:
                print(f"[{i}] EarlyStopping counter: {self.counter} out of {self.patience}")
    ...
    def save(self, i, loss, estimated_i, estimated_k=None, estimated_b=None):
        if self.verbose:
            print(f"[{i}] Loss decreased ({self.loss_min:.6f} --> {loss:.6f}).  Saving estimateds ...")
