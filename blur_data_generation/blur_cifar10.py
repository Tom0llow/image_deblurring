...
if __name__ == "__main__":
    ...
    folder_to_save = "./dataset/blured_cifar10"
    params = [0.01, 0.009, 0.008, 0.007, 0.005, 0.003]

    create_results_dir(path_to_save=folder_to_save)
    for path in tqdm(os.listdir(folder)):
        # print(path)
        trajectory = Trajectory(canvas=32, max_len=32, expl=np.random.choice(params)).fit()
        psf = PSF(canvas=32, trajectory=trajectory).fit()
