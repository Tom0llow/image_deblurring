    ...
    def __init__(self, image_path, PSFs=None, part=None, path__to_save=None):
        ...
        if PSFs is None:
            if self.path_to_save is None:
                ...
                self.PSFs = PSF(canvas=self.shape[0], path_to_save=os.path.join(self.path_to_save, "PSFs.png")).fit(save=True)
        else:
            self.PSFs = PSFs
...
if __name__ == "__main__":
    ...
    folder_to_save = "./data/blurred/celebA"
    ...
    for path in tqdm(os.listdir(folder)):
        ...
        BlurImage(os.path.join(folder, path), PSFs=psf, path__to_save=folder_to_save, part=np.random.choice([1, 2, 3])).blur_image(save=True)
