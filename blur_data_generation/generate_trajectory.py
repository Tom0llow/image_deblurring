...
class Trajectory(object):
    ...
    def fit(self, show=False, save=False):
        ...
        for t in range(0, self.iters - 1):
            ...
            dv = next_direction + self.expl * (gaussian_shake * complex(real=np.random.randn(), imag=np.random.randn()) - centripetal * x[t]) * (self.max_len / (self.iters - 1))
        ...
        x = x - complex(real=x[0].real % 1.0, imag=x[0].imag % 1.0) + complex(1, 1)
